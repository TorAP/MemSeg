...
def run(cfg):
    ...
    savedir = os.path.join(cfg['RESULT']['savedir'], cfg['EXP_NAME'],f"{wandb.config['num_training_steps']}_{wandb.config['batch_size']}_{wandb.config['lr']}")
    ...
    training(
        model              = model, 
        num_training_steps = wandb.config['num_training_steps'], 
...
sweep_configuration = {
    ...
    'parameters':{
        'batch_size': {'values': [34,64]},
        'num_training_steps':  {'values': [1170,1404]},
        'lr': {
